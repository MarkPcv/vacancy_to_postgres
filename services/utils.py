...
def get_employer_choice(employer_name: str) -> int:
    ...
    choice = int(input(f"Company name: {employer_name}\n"
                       ...
                       "1 - Yes\n"
                       "0 - No\n"
                       "or 2 - to start new search\n"))
    # Validate user choice
    while choice not in range(0, 3):
        choice = int(input("Please enter correct number either 0,1 or 2: "))
...
def chose_employers_by_user() -> list[tuple[str, str, str]]:
    ...
    while not chosen:
        ...
        search_text = input(f"Please enter search to choose "
                            f"employer ({len(chosen_employers)}/10): ")
